...
def compute_partition(p: List[Tuple[float, float]]):
                ...
                if y[concave_vertices[i]] == y[concave_vertices[j]]:
                    for k in range(len(x)):
                        if y[concave_vertices[i]] == y[k] and (
                            x[concave_vertices[i]] < x[k]
                            and x[concave_vertices[j]] > x[k]
                            or x[concave_vertices[i]] > x[k]
                            and x[concave_vertices[j]] < x[k]
                        ):
                            middles.append(k)
                    if len(middles) == 0:
                        horizontal_chords.append(
                            (concave_vertices[i], concave_vertices[j])
                        )
                middles = []
                if x[concave_vertices[i]] == x[concave_vertices[j]]:
                    for k in range(len(x)):
                        if x[concave_vertices[i]] == x[k] and (
                            y[concave_vertices[i]] < y[k]
                            and y[concave_vertices[j]] > y[k]
                            or y[concave_vertices[i]] > y[k]
                            and y[concave_vertices[j]] < y[k]
                        ):
                            middles.append(k)
                    if len(middles) == 0:
                        vertical_chords.append(
                            (concave_vertices[i], concave_vertices[j])
                        )
    ...
    for i in unmatched_concave_vertices:
        ...
        for j in max_independent:
            if j < len(horizontal_chords):
                ...
                if (
                    abs(y[i] - y[temp1]) < nearest_distance
                    and (
                        x[i] <= x[temp1]
                        and x[i] >= x[temp2]
                        or x[i] >= x[temp1]
        ...
        if nearest_distance != math.inf:
            ...
            for k in collinear_vertices:
                if (
                    x[k] == x[i]
                    and abs(y[k] - y[i]) < nearest_distance
                    and abs(k - i) != 1
                ):
                    middles = []
                    for u in range(len(x)):
                        if x[i] == x[u] and (
                            y[i] < y[u] and y[u] < y[k] or y[k] < y[u] and y[u] < y[i]
                        ):
                            middles.append(u)
                    if len(middles) == 0:
